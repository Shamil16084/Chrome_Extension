...
document.getElementById("email-data").addEventListener("click", () => {
    ...
    // Convert the data to a JSON string and encode it for use in the mail body
    const emailBody = encodeURIComponent(JSON.stringify(exportData, null, 2));

    // Construct the mailto link correctly
          const mailtoLink = `mailto:?subject=Profile Data&body=${encodeURIComponent(exportData)}`
